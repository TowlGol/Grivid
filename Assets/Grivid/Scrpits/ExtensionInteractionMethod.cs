...
namespace Assets.Imvisd.Scrpits {
    public static class ExtensionInteractionMethod {
        public static Component AddComponentExt(this GameObject obj, string scriptName,bool isClass = true) {
            Component cmpnt = null;
            for (int i = 0; i < 10; i++) {
                //If call is null, make another call
                cmpnt = _AddComponentExt(obj, scriptName, i, isClass);
        ...
        private static Component _AddComponentExt(GameObject obj, string className, int trials,bool isClass) {
            ...
            Component cmpnt;
            if (isClass)
                cmpnt = obj.AddComponent(type);
            else
                cmpnt = new GameObject().AddComponent(type);
            return cmpnt;
        }
    }
}
